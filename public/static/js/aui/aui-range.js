 ...
 (function( window, undefined ) {
    ...
    auiRange.prototype = {
        _init: function(params,callback) {
            ...
            params.element.insertAdjacentHTML('afterend','<div style="display:none" class="aui-range-tip aui-hide">'+params.element.value+'</div>');
            ...
            params.element.addEventListener("input",function(){
				...
				var a = Math.abs(params.element.value - params.element.min);
				var b = Math.abs(params.element.max - params.element.min);
				var per = (a/b)*100;
				$("#range").css( 'background', 'linear-gradient(to right, #059CFA, white '+per+'%, white)' );
            });
